...
#define MEU_ENDERECO 0X14 // eletrodos são do endereço 0x11 em diante: 0x11, 0x12, 0x13, 0x14...
#define LED PC13
...
//#define COM_FASE // ainda não implementado
...
// Começa configurado com 200Khz 6 pontos (6 pts = 2 ciclos)
float freq_sinal = 200000;  // 200kHz
int npontos_base = 3; // menor multiplo inteiro de 'sample_freq/freq_sinal'.
//Define a quantidade minima de pontos para reconstruir o menor numero possivel de ciclos do sinal.
int n_pontos_mult = 2;// fator multiplicador de 'npontos_base'. Permite adquirir mais ciclos do sinal.
int NUM_SAMPLES = npontos_base*n_pontos_mult;
...
// Importante: código fica parado durante o delay
void pisca_led(int tempo_ms){
...
/* Processa comandos recebidos pela I2C:
...
void processacomando(){
  ...
  switch (comando) {
    ...
    case 'y': // incrementa o numero de pontos amostrados (aumenta o fator de multiplicacao de ciclos amostrados)
      n_pontos_mult = n_pontos_mult+1;
      if (npontos_base*n_pontos_mult >= NUM_SAMPLES_MAX) n_pontos_mult = n_pontos_mult-1;
      NUM_SAMPLES = npontos_base*n_pontos_mult;
      break;
...
void dadopedido(){
  // Envia para o master os dados amplitude(amplit1)
  if(mediu){
    binaryFloat amplitude, fase;
    amplitude.floatingPoint = amplit1;
    Wire.write(amplitude.binary, 4);
  }
  else{ // se não mediu, envia zeros
    for(int i=0;i<4;i++) Wire.write(0x00, 1);
  }
}
